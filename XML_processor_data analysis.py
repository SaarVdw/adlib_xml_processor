...
def process_file(xml_file):
    ...
    # Collectienamen importeren
    collectienamen_df = pd.read_excel(r"C:\Users\SaarVandeweghe\PycharmProjects\registratiecheck\collectienamen_CRLB_20240912.xlsx")
    df = pd.merge(df, collectienamen_df, on='collectienaam', how='left')

    # Toeschrijvingen importeren
    toeschrijvingen_df = pd.read_excel(r"C:\Users\SaarVandeweghe\PycharmProjects\registratiecheck\toeschrijvingen.xlsx")
    toeschrijvingen_df['priref'] = toeschrijvingen_df['priref'].astype(str)
    df = pd.merge(df, toeschrijvingen_df, on='priref', how='left')
    ...
    # Data-analyses
    statistieken = {
        'Aantal kunstwerken': [df.shape[0]],
        'Unieke collecties': [df['collectienaam'].nunique()],
        'Unieke locaties': [df['plaats collectie'].nunique()],
    }

    # Aantal kunstwerken per collectie
    collectie_telling = df['collectienaam'].value_counts().reset_index()
    collectie_telling.columns = ['Collectie', 'Aantal kunstwerken']

    # Aantal kunstwerken per locatie
    locatie_telling = df['plaats collectie'].value_counts().reset_index()
    locatie_telling.columns = ['Locatie', 'Aantal kunstwerken']

    # Meest gebruikte materialen
    materialen_telling = df['materiaal'].dropna().str.split(',').explode().value_counts().reset_index()
    materialen_telling.columns = ['Materiaal', 'Aantal keer gebruikt']

    # Meest courante iconografie thema's
    iconografie_telling = df['iconografie'].dropna().str.split(',').explode().value_counts().reset_index()
    iconografie_telling.columns = ['Iconografie', 'Aantal keer vermeld']

    # Meest courante vervaardigers
    vervaardigers_telling = df['huidige toeschrijving'].dropna().value_counts().reset_index()
    vervaardigers_telling.columns = ['Vervaardiger', 'Aantal kunstwerken']

    # Excel-bestand maken
    output_excel_file = 'kunstwerken_data_analyse.xlsx'

    with pd.ExcelWriter(output_excel_file, engine='openpyxl', mode='w') as writer:
        df.to_excel(writer, sheet_name='Gegevens', index=False)
        pd.DataFrame(statistieken).to_excel(writer, sheet_name='Statistieken', index=False)
        collectie_telling.to_excel(writer, sheet_name='Per Collectie', index=False)
        locatie_telling.to_excel(writer, sheet_name='Per Locatie', index=False)
        materialen_telling.to_excel(writer, sheet_name='Materialen', index=False)
        vervaardigers_telling.to_excel(writer, sheet_name='Vervaardigers', index=False)
        iconografie_telling.to_excel(writer, sheet_name='Iconografie', index=False)

    messagebox.showinfo("Success", f"De data en analyse zijn opgeslagen in {output_excel_file}.")
